...
import me.honkling.detainment.manager.tree.Node
import me.honkling.detainment.manager.tree.Parameter
...
fun scanForCommands(manager: CommandManager, pkg: String): List<CommandNode> {
	val clazz = manager.instance::class.java
...
private fun parseSubcommand(manager: CommandManager, parent: Node, method: Method): SubcommandNode? {
	if (!validateParameters(manager, method)) {
		manager.instance.logger.warning("Skipping invalid subcommand '${method.name}'")
		return null
	}
...
private fun validateParameters(manager: CommandManager, method: Method): Boolean {
	...
	if (first !in listOf(
		...
		manager.instance.logger.warning("Invalid executor parameter. (expected Player, ConsoleCommandSender, or CommandSender. found ${first.name})")
		return false
	}
	...
	for (parameter in method.parameters.slice(1 until method.parameters.size)) {
		...
		if (hasOptional && isRequired) {
			manager.instance.logger.warning("Found required parameter after an optional parameter")
			return false
		}
...
private fun validateParameter(manager: CommandManager, parameter: java.lang.reflect.Parameter): Boolean {
	...
	if (type !in manager.types) {
		manager.instance.logger.warning("Found parameter '${parameter.name}' using an invalid type '${type.name}'")
		return false
	}
...
private fun isSubcommand(manager: CommandManager, method: Method): Boolean {
	...
	if (isPublic && !isStatic) {
		manager.instance.logger.warning("Found a public non-static method '${method.name}'. Please make this static (to be registered as a subcommand) or private (to hide this warning)")
		return false
	}
